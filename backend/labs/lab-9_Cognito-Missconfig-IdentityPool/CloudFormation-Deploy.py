...
def deployLabEnvironment(access_key: str, secret_key: str, region:str="us-east-1"):
    ...
    # ──────────────────────────────────────────────────────────────
    STACK_NAME    = "Missconfigured-IdentityPool"
    ...
    try:
        ...
        s3.put_bucket_tagging(
            ...
            Tagging={
                "TagSet": [
                    {"Key": "cloud-hacking-labs", "Value": "Missconfigured-IdentityPool"},
                ]
            }
        )
    ...
    with open(auth_config_file, "w") as f:
        ...
        f.write(f'"IdentityPoolId": "{identity_pool_id}"\n}}')
    ...
    try:
        ...
        # Se establece la contraseña para evitar el challenge de cambio de contraseña al primer inicio
        cognito.admin_set_user_password(
            UserPoolId=user_pool_id,
            Username="admin@shoplab.com",
            Password="jha873gcn81rnancpag",
            Permanent=True
        )
    ...
    # ──────────────────────────────────────────────────────────────
    # 10. Subir credenciales sensibles del usuario admin al bucket 
    # ──────────────────────────────────────────────────────────────
    env_bucket = outputs.get("SensitiveAdminDataBucketName")
    if not env_bucket:
