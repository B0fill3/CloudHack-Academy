...
def initialize_db():
    with app.app_context():
        ...
        if not User.query.first():
            admin_user = User(username='admin', password='admin', email='admin@shoplab.com', admin=True)
            db.session.add(admin_user)
            user1 = User(username='david', password='james', email='davidj@notshoplab.com', admin=False)
