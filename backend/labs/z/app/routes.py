from flask import render_template, redirect, url_for, request, flash, abort, session,render_template_string, make_response
...
def generate_invoice():
    """
    Genera la factura en PDF usando los datos almacenados, incrustando
    comentarios del usuario que podrían incluir un iframe malicioso.
    """
    ...
    order_data = {
        'product': 'Sneaker XYZ',
        'quantity': 2,
        'comments': '<iframe src="https://ipinfo.io/json"></iframe>'
    }

    # Plantilla HTML de la factura. Aquí insertamos directamente los comentarios
    # sin ningún tipo de sanitización, provocando la vulnerabilidad.
    html_template = """
    <html>
      <head><meta charset="utf-8"></head>
      <body>
        <h1>Factura de Pedido #{{ order_id }}</h1>
        <p>Producto: {{ product }}</p>
        <p>Cantidad: {{ quantity }}</p>
        <p>Comentarios:</p>
        <!-- Aquí se inyectan directamente los comentarios, que pueden contener un iframe -->
        {{ comments | safe }}
      </body>
    </html>
    """

    # Renderiza la plantilla en un string
  ...
  # Opciones para wkhtmltopdf a través de pdfkit
    options = {
        'enable-javascript': '',
        'javascript-delay': '2000',  # 2 segundos de retraso para que cargue el contenido del iframe
    }
