...
def deployIAMEnvironment(access_key: str, secret_key: str, region:str="us-east-1"):
    """
    ...
    # ──────────────────────────────────────────────────────────────
    STACK_NAME    = "iam-environment"
    TEMPLATE_FILE = Path("aws/IAM_Environment/IAM_env.yml")

    account_id = sts.get_caller_identity()["Account"]
    bucket_name = f"cloudformation-lab-templates-{account_id}-{region}"
    object_key  = f"{STACK_NAME}/{int(time.time())}-template.yml"
    ...
    try:
        ...
        s3.put_bucket_tagging(
            ...
            Tagging={
                "TagSet": [
                    {"Key": "cloud-hacking-labs", "Value": "IAM-Environment"},
                ]
            }
        )
    ...
    try:
        ...
        cf.create_stack(
            ...
            Tags        = [{"Key": "cloud-hacking-labs", "Value": "IAM-Environment"}],
        )
...
def getEnvironmentStatus(access_key: str, secret_key: str, region:str="us-east-1"):
    """
    Busca un stack que tenga la etiqueta cloud‑hacking‑labs = IAM-Environment.
    Devuelve:
      {
        "stackName": "iam-lab",
    ...
    for page in paginator.paginate():
        for summary in page["StackSummaries"]:
            ...
            try:
                ...
                if tags.get("cloud-hacking-labs") == "IAM-Environment":
